...
def _normalize_image_origin(url: str | None) -> str | None:
    ...
    signature. The query string (resize/quality/strip params - never identity) is always
    stripped. Returns None for an empty/unparseable URL - never raises, never a false "match"."""
    if not url:
        return None
    try:
        parsed = urlsplit(url)
...
# Applied ONLY when two candidates additionally share the same normalized image origin (see
# _normalize_image_origin()) - never applied globally. Calibrated against real, measured data: the
# 4 real same-origin genuine-duplicate pairs in this dataset (Honor Robot Phone's own direct-vs-
# Photon-CDN pair, plus 3 other real 9to5mac/9to5google direct-vs-Photon pairs) measured Hamming
# distances of 1, 2, 4, and 9; the real same-BASE-FILE-but-genuinely-DIFFERENT-crop pairs in this
# same dataset (Guardian's own precrop-driven variants - a different visible region of the same
# master photo, not a resize) measured 22-30 - a wide, clean, real-evidence gap. 10 sits just above
# the highest confirmed-duplicate distance and well below the lowest confirmed-distinct-crop
# distance - not a guess, not the same as blindly raising the global 4 (see _NEAR_DUPLICATE_MAX_
# HAMMING_DISTANCE, imported from services/media_ranking.py, completely unchanged for every other
# case, including cross-story reuse).
_SAME_ORIGIN_NEAR_DUPLICATE_MAX_HAMMING_DISTANCE = 10


def _compute_within_event_duplicate_flags(candidates: list[EditorialImageCandidate]) -> dict[UUID, bool]:
    """Phase 23.1Q media-quality corrective phase: the earliest-appearing (best-ranked, since
    `candidates` already arrives ordered by the existing `rank` column) instance of any exact or
    near-identical image always survives; only later occurrences in the same set are flagged.
    ...
    divergent duplicate-detection rule for the general case. Exact sha256 match is checked first
    (cheaper, unambiguous); perceptual distance next, at the existing global threshold; then, only
    when the two candidates ALSO share the same normalized image origin (NEWS Output Stability
    Fix, Case E - see _normalize_image_origin()'s own docstring for the real evidence), the wider
    _SAME_ORIGIN_NEAR_DUPLICATE_MAX_HAMMING_DISTANCE applies instead - never origin-match alone
    without compatible hash evidence too. A candidate with no hash at all is never flagged -
    absence of evidence is not evidence of duplication."""
    seen_sha256: set[str] = set()
    seen: list[tuple[str, str | None]] = []  # (perceptual_hash, normalized_origin) of survivors
    ...
    for candidate in candidates:
        ...
        if candidate.sha256 and candidate.sha256 in seen_sha256:
            ...
            for known_hash, known_origin in seen:
                distance = hamming_distance(candidate.perceptual_hash, known_hash)
                if distance <= _NEAR_DUPLICATE_MAX_HAMMING_DISTANCE:
                    is_duplicate = True
                    break
                if origin is not None and origin == known_origin and distance <= _SAME_ORIGIN_NEAR_DUPLICATE_MAX_HAMMING_DISTANCE:
                    is_duplicate = True
                    break
        ...
        if not is_duplicate:
            ...
            if candidate.perceptual_hash:
                seen.append((candidate.perceptual_hash, origin))
    return flags
