"""Phase 23.1P - the final 2-hour bounded LIVE acceptance canary for the Story Memory/Quotes/
Editorial-Gate closure work (docs/phase23_1p_story_memory_quotes_gate_report.md).

Reuses scripts/_phase23_1o_2h_combined_news_canary.py's own proven structure verbatim (settings
applied IN-PROCESS ONLY, hard preflight route assertion, the true per-message hard delivery cap
from scripts/_canary_delivery_cap.py, real production-cadence multi-stage scheduling) - every
runtime protection from that canary carries over unchanged. Three differences only:

1. `settings.story_memory_mode = "shadow"` (IN-PROCESS ONLY, restored to whatever `.env` has the
   instant this process exits - never written to disk) - so Story Memory genuinely runs for real
   this time, and `services/story_duplicate_guard.py`'s already-wired delivery-time guard has real
   data to consult. `telegram_story_reply_mode` is deliberately left untouched (stays "off",
   whatever `.env` has) - Part J's own "do not invent a new visual format" instruction is satisfied
   for free that way: an allowed STORY_UPDATE simply sends as an ordinary fresh root post, unchanged.
2. A new `_MAX_ANALYZED` cap (100) - Phase 23.1O had none; Part J requires one this time.
3. Editorial-treatment/quote-sourcing fixes are plain code changes (services/editorial_treatment.py,
   capabilities/copywriting_capability.py, capabilities/executor.py) - already active by construction,
   no settings flag to flip for them.
"""
import asyncio
import json
import logging
...
async def main() -> None:
    ...
    settings.story_memory_mode = "shadow"  # IN-PROCESS ONLY - restored in the finally block below
    ...
    for key in ("editorial_delivery_mode", "copywriting_prompt_version", "fact_safety_mode",
                "story_memory_mode", "telegram_story_reply_mode", "article_acquisition_mode",
    ...
    try:
        ...
        while True:
            ...
            _dump_state(
                "scripts/_phase23_1p_2h_canary_state.json",
                extra={
        ...
        print("=== WROTE scripts/_phase23_1p_2h_canary_state.json ===")
