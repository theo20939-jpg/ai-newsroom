"""Phase B.5R.3: founder review sheets (pixels first, minimal text) + the immersive preservation proof.

  hero_object_stage_fidelity.png, culture_collage_fidelity.png, family_fidelity_master.png
  immersive_preservation.json   (B.5R.3 renderer output vs the accepted B.5R.1 immersive PNGs, byte for byte)
...
TEXT = {
    fam.HERO: (
        "Objects now cross the frame: cropped by 1-4 canvas edges (moon, phone macro, panel-cropped foldable and phone, a full-height power bank), 48-100% of the canvas; "
        "two dark grounds, two graphite-panel mixed grounds, two light; six different arrangements (bottom-right, full-frame, right macro, top band + panel, left panel + numeral column, full-height right bleed); "
        "type is tiny, poster-scale on the object's own ground, or a giant numeral.",
        "Only a handful of real objects sit on a uniform ground and there are no true cut-outs, so the light grounds remain cleaner than the reference's dark cinematic product shots; "
        "no glow, reflection or light integration on the object; two light variants (C, F) still rest on a white photo ground.",
    ),
    fam.COL: (
        "Four different mechanics: reaction/punchline (punchline on top, the one reaction image below), mixed social editorial (die-cut cut-out + evidence, the only starburst), "
        "poll/choice UI (the option-card stack is the mechanic), screenshot chaos (five uneven real screenshots, hand-drawn box/arrow annotations, no reaction image). Three different surfaces; "
        "a different device set per composition; clear primary > secondary > tertiary.",
        "Torn edges are a generated jagged outline (more regular than real paper); no emoji or hand-drawn sticker art; the fragments lean on UI and screenshot material; C and D keep more empty canvas "
        "than the reference's densest tiles; still a designed layout rather than raw internet mess.",
    ),
}
...
def family_sheet(dna, out: Path, family_id: str) -> None:
    ...
    d.text((24, 16), f"{title} - reference members vs our real-media reconstructions (B.5R.3)", font=ig_font(34, "black"), fill=(240, 241, 244))
...
def master_sheet(dna, out: Path, b5r1_dir: Path) -> None:
    ...
    d = ImageDraw.Draw(sheet)
    d.text((20, 18), "FAMILY FIDELITY (B.5R.3) - reference members vs our real-media reconstructions (renderer fixtures, neutral copy, existing media)", font=ig_font(28, "black"), fill=(240, 241, 244))
...
def immersive_preservation(out: Path, b5r1_dir: Path) -> dict:
    """Re-render the accepted immersive plans with the B.5R.3 renderer and compare bytes with the accepted B.5R.1 PNGs."""
    rows = []
