"""Phase 23.1J - golden real-data replay of Copywriting V8 (docs/
phase23_1j_copywriting_v8_report.md §15). Calls the real V8 CopywritingCapability directly against
5 real, already-triaged NewsEvent rows, using their already-persisted NEWS_ANALYSIS research/
intelligence output as input - mirrors scripts/phase19_overnight_abc_harness.py's own established
"manual CapabilityContext construction, direct capability.execute() call" pattern, used here
because each of these 5 events already has a real CONTENT_GENERATION EditorialTask from an earlier
phase, and services/workflow_service.py::create_task() unconditionally refuses a second task for
the same (event, workflow_type) pair regardless of status - so the normal run_content_generation_
for_event() path (which always creates a fresh task) cannot be reused for these specific events.
...
settings.copywriting_prompt_version is set to "8" in-process only, for the duration of this
...
async def main() -> None:
    setup_logging()
    settings.copywriting_prompt_version = "8"
            ...
            async with session.begin_nested():
                for call in result.calls:
                    await record_ai_execution(
                        session, task_id=na_task.id, event_id=event.id, workflow_name="content_generation_v8_replay",
    ...
    with open("scripts/_phase23_1j_golden_replay_results.json", "w", encoding="utf-8") as f:
        json.dump({"cost_delta_usd": str(cost_delta), "results": results}, f, indent=2, ensure_ascii=False, default=str)
    print("=== WROTE scripts/_phase23_1j_golden_replay_results.json ===")
