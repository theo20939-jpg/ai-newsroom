...
Three phases, run independently via the `--phase` flag:
  ...
  live-send - Phase C: links a SINGLE, already-real, already-completed CONTENT_GENERATION
              ...
              run_shadow_discovery for that same real event) and sends ONE real, combined
              news+image Telegram message to settings.editorial_chat_id via
              services.image_preview_notifier.send_news_with_image_preview (docs/
              phase16_ux_combined_preview_fix_report.md) - zero new LLM calls (the ContentDraft
              already existed from the system's own past, unrelated real operation), zero
              synthetic events.
...
async def run_live_send_phase(storage_root: str, event_id: str) -> dict:
    ...
    async with async_session_factory() as session:
        ...
        bot = create_bot()
        try:
            outcome = await send_news_with_image_preview(
                bot, settings.editorial_chat_id, session, draft=draft_read, event=event, dry_run=False,
            )
        ...
        return {
            "status": "sent" if outcome.sent else "send_failed",
            "has_image": outcome.has_image,
