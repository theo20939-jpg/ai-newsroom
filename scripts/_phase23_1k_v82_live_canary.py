"""Phase 23.1K - the V8.2 live editorial canary (docs/phase23_1k_v82_live_canary_report.md).
Same one-shot, host-run, bounded design as scripts/_phase23_1i_5_news_hardened_canary.py (mirrored
closely) - NOT a persistent worker, NOT a Docker container, canary settings applied IN-PROCESS
ONLY, hard preflight route assertion before any LLM/Telegram call.

Differences from the Phase 23.1I canary, per docs/phase23_1k_v82_live_canary_preflight.md:
- copywriting_prompt_version="8.2" (not "7") - the first live canary to actually exercise the
  V8-family dispatch wired into worker/content_cycle.py this same phase (Task 23.1K.2).
- story_memory_mode is left exactly as the real .env has it ("off") - not touched, not exercised
  this run (out of scope per the phase brief - "do not touch Story Memory algorithm"). Because of
  this, automation_worker is NOT paused this run (no need to isolate a shadow Story Memory
  observation window like Phase 23.1I did) - it keeps running normally, doing real collection +
  triage in the background exactly as always. This script therefore does NOT call
  services.triage_orchestrator.run_triage_cycle() itself (that would race automation_worker's own
  triage, which the Phase 23.1I script's own docstring already documented as a losing race) - it
  only calls run_analysis_cycle()/run_content_cycle() each round, consuming whatever
  automation_worker has already triaged.
- The duplicate-delivery guard (services/story_duplicate_guard.py) stays wired in and monitored,
  unchanged from Phase 23.1I - not touched this phase.
- Additional V8-family-specific per-sent-post fields recorded: is_v8_family_output, rendered HTML
  length, paragraph count of the rendered card, whether a photo or text-only message was sent.

Hard bounds (identical to Phase 23.1I): max 50 analyzed events, max 5 delivered NEWS posts, max 4
hours runtime, max $1 incremental cost. Hard route safety: chat_id=-1004297182444, topic=2 only.
"""
import asyncio
import json
import logging
...
async def main() -> None:
    ...
    # story_memory_mode: intentionally NOT set - stays whatever the real .env already has
    # ("off", confirmed by docs/phase23_1k_v82_live_canary_preflight.md §4). Honestly disclosed,
    ...
    with (
        ...
        for round_num in range(1, _MAX_ROUNDS + 1):
            ...
            print(f"=== ROUND {round_num}: content cycle (LIVE, router mode, V8.2) ===")
    ...
    async with async_session_factory() as session:
        ...
        for task in content_tasks:
            ...
            matching_send = next(
                (s for s in _recorded_sends if event and event.url and (
                    (isinstance(s.get("text"), str) and event.title and event.title[:20] in s["text"])
                )), None,
            )
            ...
            records.append({
                ...
                "is_v8_family_output": v8_family,
                "copywriting_output": copywriting_output,
                "rendered_html": rendered_html,
                "rendered_html_len": len(rendered_html) if rendered_html else None,
                "rendered_paragraph_count": (rendered_html.count("\n\n") + 1) if rendered_html else None,
                "send_method": matching_send.get("method") if matching_send else None,
                "send_has_button": matching_send.get("has_button") if matching_send else None,
                "send_message_id": matching_send.get("message_id") if matching_send else None,
                "incremental_cost_usd": str(event_cost),
            })

    with open("scripts/_phase23_1k_records.json", "w", encoding="utf-8") as f:
        json.dump({
    ...
    print("=== WROTE scripts/_phase23_1k_records.json ===")
