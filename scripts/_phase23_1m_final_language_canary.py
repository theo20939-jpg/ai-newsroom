"""Phase 23.1M final polish - the bounded LIVE LANGUAGE canary, run only after the golden replay
showed a clear improvement on the Flock/anti-recognition headline case (docs/
phase23_1m_final_russian_polish_report.md).

Reuses scripts/_phase23_1m_language_canary.py's own structure verbatim (itself inherited from
scripts/_phase23_1l_final_local_acceptance_canary.py: settings applied IN-PROCESS ONLY, hard
preflight route assertion, the true per-message hard delivery cap from
scripts/_canary_delivery_cap.py, provenance logging, the Part N duplicate tripwire) - every
runtime protection from Phase 23.1L carries over unchanged. The only difference this phase:
copywriting_prompt_version="8.4" (the new, frozen-after-this-phase Russian-naturalness polish
version; v8.3 stays untouched). Bounds identical to the V8.3 language canary (max 30 analyzed,
max 3 deliveries HARD LIMIT, max $0.50). story_memory_mode is NOT touched - stays exactly
whatever the real .env has ("off"), preserving the current approved state per the phase brief's
...
async def main() -> None:
    ...
    settings.copywriting_prompt_version = "8.4"  # FROZEN after this phase, never modified again
    ...
    with patch.object(content_cycle, "_classify_event_for_router_treatment", _instrumented_classify):
        for round_num in range(1, _MAX_ROUNDS + 1):
            ...
            print(f"=== ROUND {round_num}: content cycle (LIVE, router mode, V8.4, capped at {cap.remaining} remaining) ===")
    ...
    with open("scripts/_phase23_1m_final_language_canary_records.json", "w", encoding="utf-8") as f:
        json.dump({
    ...
    print("=== WROTE scripts/_phase23_1m_final_language_canary_records.json ===")
