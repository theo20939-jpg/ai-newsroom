...
# the Director writes angle / why_now in Russian whatever the source language (tests/test_instagram_daily_premise_guard.py holds the
# independently worded RU- and EN-source versions; this probe uses them)
from tests.test_instagram_daily_premise_guard import GYM_DIFFERENT_ANGLE, GYM_EN, GYM_RU  # noqa: E402
...
async def main() -> dict:
    ...
    monday = state.InstagramEditorialHistoryItem(
        ...
        created_at=datetime(2026, 8, 10, 12, tzinfo=timezone.utc), angle=GYM_RU["angle"], angle_intent=GYM_RU["intent"],
        why_now=GYM_RU["why_now"],
    )

    async def check(story, d):
        return await state.check_instagram_editorial_duplicate(None, source_story_id=story, angle=d["angle"], angle_intent=d["intent"],
                                                               why_now=d["why_now"], now=now)
    ...
    try:
        same_story = await check(RU_STORY, GYM_RU)
        other_language = await check(EN_STORY, GYM_EN)
        different_angle = await check(EN_STORY, GYM_DIFFERENT_ANGLE)
    finally:
    ...
    return {
        ...
        "different_angle_control": {"blocked": different_angle.blocked, "reason": different_angle.reason},
