"""Phase 23.1L Part J - the FINAL LOCAL acceptance canary before Phase 23.2 VPS preparation (docs/
phase23_1l_runtime_isolation_final_canary_report.md).

Builds directly on scripts/_phase23_1k_v82_live_canary.py's own structure (settings applied
IN-PROCESS ONLY, hard preflight route assertion, `_instrument_send_message`/`_instrument_send_photo`
recording pattern) with three additions specific to this phase:

1. A TRUE per-message hard delivery cap (scripts/_canary_delivery_cap.py, Part F/G) wraps the bot
   BEFORE any round runs - `HardDeliveryCap.try_consume()` is checked immediately before every
   individual bot.send_message/send_photo call, not per-batch/round/after-send. This is the actual
   fix for the Phase 23.1K incident (6 sends against max_deliveries=5).
2. Provenance logging per candidate (Part K): event_id, source id, source URL, collected_at,
   source type, upstream publication timestamp - logged before content generation, so a reviewer
   can independently confirm every processed event is a real collected Newsroom row, not a
   synthetic fixture (structurally guaranteed anyway by Phase 23.1L Part B/C's DB isolation, but
   logged explicitly per the brief's own instruction).
3. A simple, transparent, OBSERVATIONAL duplicate tripwire (Part N) - compares each new candidate's
   normalized title against every title already seen this run; on a high-overlap match, STOPS
   before sending the second one and records both events + whatever real Story Memory
   classification exists (story_memory_mode stays "off" this run, so this is typically "none
   computed", disclosed honestly, never fabricated). This is NOT a Story Memory change and NOT a
   general-purpose matcher - a narrow, disclosed, canary-only safety observation.

copywriting_prompt_version="8.2" (FROZEN this phase, never modified). story_memory_mode is NOT
touched - stays exactly whatever the real .env has ("off", confirmed in docs/
phase23_1l_final_canary_preflight.md).
"""
import asyncio
import json
import logging
...
_TARGET_MESSAGES_MIN = 3
_TARGET_MESSAGES_MAX = 5
_MAX_ROUNDS = 10
_MAX_COST_USD = 1.00
...
async def main() -> None:
    ...
    settings.copywriting_prompt_version = "8.2"  # FROZEN - never modified this phase
    ...
    with patch.object(content_cycle, "_classify_event_for_router_treatment", _instrumented_classify):
        for round_num in range(1, _MAX_ROUNDS + 1):
            ...
            print(f"=== ROUND {round_num}: content cycle (LIVE, router mode, V8.2, capped at {cap.remaining} remaining) ===")
    ...
    with open("scripts/_phase23_1l_final_canary_records.json", "w", encoding="utf-8") as f:
        json.dump({
    ...
    print("=== WROTE scripts/_phase23_1l_final_canary_records.json ===")
