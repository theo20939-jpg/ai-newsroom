"""SOCIAL-INTELLIGENCE-INTEGRATION-1, spec §13/§23: pure Director Console rendering. No aiogram/Bot
type, no database access - mirrors bot/business_context_formatting.py's own "pure functions, no
database access" discipline exactly. Russian, user-facing, compact (spec §23) - no internal DB IDs
unless genuinely useful for debugging context, no giant JSON dumps."""
...
def render_directors_status(status: DirectorConsoleStatus) -> str:
...
def render_plan(view: PlanView, *, platform: str | None = None) -> str:
...
def render_opportunities(view: OpportunitiesView, *, platform: str | None = None) -> str:
    ...
    for row in shown:
        ...
        if row.restricted_claims:
            lines.append("Запрещённые утверждения: " + ", ".join(row.restricted_claims))
...
def render_calendar(view: CalendarView, *, platform: str | None = None) -> str:
...
def render_performance(view: PerformanceView, *, platform: str | None = None) -> str:
