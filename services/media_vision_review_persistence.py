...
MEDIA-PROD-1: also the sole place that READS MediaVisionReview rows back
(`get_media_vision_review_calibration_summary()` below) - a plain, read-only diagnostics query
over whatever the manual harness has already accumulated, for a human operator to calibrate a
FUTURE enforcement mode's thresholds against real reviewer verdicts rather than guessing. This is
explicitly NOT wired into any automatic/scheduled path (worker/content_cycle.py is untouched by
this phase) and has no side effects of any kind - it cannot reject, filter, or otherwise affect any
delivery decision. `media_vision_review_mode` stays exactly the two-state `off`/`shadow` it already
was (core/config.py's own documented "regardless of this setting's value, always requires the
manually-invoked harness script" fence is preserved, not removed)."""
