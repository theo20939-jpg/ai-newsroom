...
class RedisCostTracker:
    ...
    def _compute_actual_cost(self, call: CapabilityCall) -> Decimal:
        if call.model_used is None:
            return Decimal("0")
        tier = self._pricing_catalog.get_tier(call.model_used, condition="standard")
    ...
    async def record(self, task_id: UUID, capability_name: str, call: CapabilityCall) -> None:
        cost = self._compute_actual_cost(call)
