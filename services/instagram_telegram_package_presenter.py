...
def _creative_execution_lines(package: InstagramContentPackage) -> list[str]:
    media_plan = package.media_plan if isinstance(package.media_plan, dict) else {}
    plan = media_plan.get("creative_execution_plan")
    execution = media_plan.get("media_execution") or {}
    if not isinstance(plan, dict) or not plan:
        return []
    values = [
        ("КРЕАТИВНАЯ ИДЕЯ", plan.get("main_idea")),
        ("ФОКУС", plan.get("focal_point")),
        ("МЕДИА", execution.get("strategy") or plan.get("media_strategy")),
        ("ПОЧЕМУ ТАК", plan.get("media_rationale")),
        ("КОМПОЗИЦИЯ", plan.get("composition_direction")),
        ("ВИЗУАЛЬНЫЙ ПОДХОД", plan.get("visual_treatment")),
        ("БРЕНДИНГ", plan.get("branding_treatment")),
        ("ИЗБЕГАЕМ ПОВТОРА", plan.get("avoid_recent_treatment")),
    ]
    return [f"<b>{label}:</b> {_esc(str(value))}" for label, value in values if value]
