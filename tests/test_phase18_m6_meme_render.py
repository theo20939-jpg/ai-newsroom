...
# ---------------------------------------------------------------------------
# MEME-PROD-4: 4-quadrant panel caption rendering (MemeCopy.panel_texts).
# ---------------------------------------------------------------------------

_PANEL_COPY = MemeCopy(
    top_text="Строит план",  # schema-required, equals panel_texts[0], never separately rendered
    bottom_text=None,
    panel_texts=["Строит план", "Ждёт GTA 6", "PS5 Pro распродан", "Продаёт почку"],
    punchline_short="p", telegram_caption="c", alt_text="alt",
)


def _four_quadrant_image(size: int = 1024) -> Image.Image:
    from PIL import ImageDraw as _ImageDraw

    image = Image.new("RGB", (size, size))
    draw = _ImageDraw.Draw(image)
    half = size // 2
    draw.rectangle((0, 0, half, half), fill=(90, 110, 150))
    draw.rectangle((half, 0, size, half), fill=(200, 190, 170))
    draw.rectangle((0, half, half, size), fill=(60, 80, 60))
    draw.rectangle((half, half, size, size), fill=(220, 220, 220))
    return image


def test_render_meme_uses_panel_path_when_panel_texts_present(tmp_path) -> None:
    storage = LocalImageStorage(tmp_path)
    buf = BytesIO()
    _four_quadrant_image().save(buf, format="PNG")

    result = render_meme(buf.getvalue(), _PANEL_COPY, storage=storage)

    assert result.status == MemeRenderStatus.RENDERED
    assert result.top_text_contrast_ratio is None  # classic bands not used for this shape
    assert result.bottom_text_contrast_ratio is None
    assert result.panel_text_contrast_ratios is not None
    assert len(result.panel_text_contrast_ratios) == 4
    assert result.contrast_passed is True


def test_render_meme_classic_path_unaffected_when_panel_texts_absent(tmp_path) -> None:
    """The existing top/bottom path stays byte-identical - panel_text_contrast_ratios is simply
    None, nothing else changes."""
    storage = LocalImageStorage(tmp_path)
    result = render_meme(_generate_base_image(), _VALID_COPY, storage=storage)
    assert result.panel_text_contrast_ratios is None
    assert result.top_text_contrast_ratio is not None


def test_draw_panel_captions_writes_visibly_distinct_pixels_in_each_quadrant() -> None:
    from PIL import ImageDraw as _ImageDraw

    from services.meme_render import _draw_panel_captions

    image = _four_quadrant_image()
    original = image.copy()
    draw = _ImageDraw.Draw(image)
    ratios, violations = _draw_panel_captions(image, draw, _PANEL_COPY.panel_texts)
    ...
    half = image.width // 2
    quadrant_boxes = [(0, 0, half, half), (half, 0, image.width, half), (0, half, half, image.height), (half, half, image.width, image.height)]
    for box in quadrant_boxes:
        before = list(original.crop(box).getdata())
        after = list(image.crop(box).getdata())
        assert before != after  # each quadrant actually received its own caption


def test_draw_panel_captions_reserves_watermark_clearance_in_bottom_right_quadrant_only() -> None:
    """Mirrors test_reserve_watermark_clearance_keeps_corner_region_untouched's own technique for
    the classic path - the bottom-right quadrant's own bottom-right corner (where
    apply_nnj_watermark() will place the mark for a panel meme, since bottom_text is always null)
    must stay untouched by caption text, even for a caption long enough to otherwise reach it."""
    from PIL import ImageDraw as _ImageDraw
    ...
    draw = _ImageDraw.Draw(image)
    long_panel_texts = [
        "Короткий текст", "Короткий текст", "Короткий текст",
        "Это очень длинная подпись для последней панели мема",
    ]
    _draw_panel_captions(image, draw, long_panel_texts)

    # Bottom-right quadrant: quad_w=quad_h=512, margin=30, right_margin=30+round(512*0.22)=143,
    # so caption text is centered within x:[542,881] at most - real production watermark footprint
    # for a 1024-wide canvas is roughly x:[916,998] y:[966,998] (services/meme_watermark.py's own
    # _WATERMARK_WIDTH_FRACTION=0.08/_MARGIN_FRACTION=0.025). This box starts safely beyond the
    # computed max text edge (881) and covers the real watermark footprint with margin, mirroring
    # test_reserve_watermark_clearance_keeps_corner_region_untouched's own "generous safety box,
    # not the exact footprint" technique for the classic path.
    watermark_corner_box = (910, 960, 1024, 1024)
    region_pixels = list(image.crop(watermark_corner_box).getdata())
    assert all(pixel == background for pixel in region_pixels)
