...
def test_hero_curve_anchors_are_the_supplied_series_verbatim() -> None:
    ...
    assert "_segmented_anchor_path(" in body  # clean segmented editorial line, no synthetic points
...
def test_hero_has_no_chart_below_two_points_and_never_a_crude_polyline() -> None:
    ...
    assert "_segmented_anchor_path(" in spark and "Image.Resampling.LANCZOS" in spark
...
def test_hero_value_unit_label_use_the_real_bold_face() -> None:
    ...
    assert 'data_weight="semibold"' in body  # label (FINAL-BOARD-MATCH-7: SemiBold, not Bold)
