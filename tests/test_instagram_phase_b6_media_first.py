...
def test_v10_makes_generated_media_first_class_and_v91_is_untouched() -> None:
    ...
    assert cd.CAROUSEL_PROMPT_VERSION == "10.8" and "10" in cd.MEDIA_FIRST_CAROUSEL_VERSIONS and "10.1" in cd.MEDIA_FIRST_CAROUSEL_VERSIONS and "10.1" in cd._EVIDENCE_HANDLE_CAROUSEL_VERSIONS
...
_FIXTURE_BODIES = {
    "hook": "Для простой задачи хватает небольшой модели, а мощная всё это время простаивает.",
    "evidence": "Ввод, действие, результат: короткий запрос проходит путь без флагмана и без очереди.",
    "takeaway": "Возьми типовую задачу и сравни, во что она обходится на разных моделях.",
}
...
def _slide_dict(role, copy_text, source, regions, *, brief=None, family="light_utility_editorial", subject=None, **kw):
    ...
    base = {"role": role, "slide_copy": copy_text, "source_evidence": "E1", "slide_purpose": role, "media_need": None,
            ...
            # content pass (v10.7): every fixture slide says something concrete on its own, like a real plan must - and (v10.8) something
            # different from every other card
            "slide_body": _FIXTURE_BODIES.get(role, "Для простой задачи хватает небольшой модели, а мощная всё это время простаивает.")}
    base.update(kw)
    return base
...
async def test_live_trigger_runs_the_media_first_pipeline_end_to_end(db_session, monkeypatch, tmp_path) -> None:
    ...
    assert obs["prompt_version"] == "10.8" and obs["text_only_slides"] == [] and obs["typographic_final_media_slides"] == []
