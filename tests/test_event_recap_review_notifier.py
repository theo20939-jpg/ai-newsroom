"""NINJA PULSE RECAP Phase R2 integration, Phase C.1: services.event_recap_review_notifier.
send_event_recap_review(). Pure unit tier, mirrors tests/test_telegram_editorial_routing.py's own
established convention exactly: `Bot` is a plain `unittest.mock.AsyncMock` - no real Postgres, no
real Telegram API contact of any kind, including a "live" (`dry_run=False`) send, since the fake
bot never leaves the process.
"""
from __future__ import annotations

from dataclasses import replace
from unittest.mock import AsyncMock

import pytest
from aiogram.enums import ParseMode

from core.config import settings
from schemas.editorial_route import EditorialDestination
from services.event_recap import FactVerificationResult
from services.event_recap_review_notifier import send_event_recap_review
from tests.test_event_recap import _candidate_from_titles


@pytest.fixture(autouse=True)
def _routing_settings(monkeypatch: pytest.MonkeyPatch) -> None:
    """Isolated topic ids for every test in this file - mirrors tests/test_telegram_editorial_
    routing.py's own identical fixture, never the real environment's values."""
    monkeypatch.setattr(settings, "newsroom_telegram_chat_id", -100123456789)
    monkeypatch.setattr(settings, "telegraph_topic_id", 33)


def _synthesized_candidate():
    """A real, in-memory EventRecapCandidate (tests.test_event_recap's own established helper),
    with the post-synthesis fields filled in - exactly the shape EventRecapCapability.execute()
    would hand back after a real synthesize_event_recap() call."""
    candidate = _candidate_from_titles(
        ["Apple unveils new Watch Ultra priced at $999"], "Apple unveils new Watch Ultra",
    )
    return replace(
        candidate,
        recap_title="Apple Watch Ultra Unveiled",
        recap_summary="Apple introduced a new Watch Ultra model.",
        key_takeaways=["Priced at $999.", "Positioned as the flagship Watch tier."],
        uncertainty_notes=["Availability date not yet confirmed."],
        fact_verification=FactVerificationResult(
            status="pass", claims_checked=2, supported=2, uncertain=0, unsupported=0, flagged_claims=[],
        ),
    )


@pytest.mark.asyncio
async def test_dry_run_is_true_by_default_and_never_calls_send_message() -> None:
    bot = AsyncMock()
    candidate = _synthesized_candidate()

    outcome = await send_event_recap_review(bot, candidate)

    bot.send_message.assert_not_called()
    assert outcome.sent is False
    assert outcome.reason == "dry_run"


@pytest.mark.asyncio
async def test_live_send_builds_a_correct_preview_message() -> None:
    bot = AsyncMock()
    bot.send_message.return_value.message_id = 777
    candidate = _synthesized_candidate()

    outcome = await send_event_recap_review(bot, candidate, dry_run=False)

    assert bot.send_message.call_count == 1
    args, kwargs = bot.send_message.call_args
    sent_chat_id, sent_text = args[0], args[1]
    assert sent_chat_id == -100123456789
    assert kwargs["message_thread_id"] == 33  # EditorialDestination.TELEGRAPH's own topic id

    assert candidate.recap_title in sent_text
    assert candidate.recap_summary in sent_text
    for takeaway in candidate.key_takeaways:
        assert takeaway in sent_text
    for note in candidate.uncertainty_notes:
        assert note in sent_text
    assert "пройдена" in sent_text  # FactVerificationResult(status="pass")'s own RU label
    assert "shadow" in sent_text.lower()  # never rendered as a finished, publishable post

    assert outcome.sent is True
    assert outcome.destination is EditorialDestination.TELEGRAPH
    assert outcome.message_id == 777


@pytest.mark.asyncio
async def test_uses_the_real_send_to_editorial_destination_routing() -> None:
    """Proves this notifier routes through the existing, unmodified send_to_editorial_destination()
    - never a parallel/duplicated Telegram call - by checking the real routing side effects
    (parse_mode, message_thread_id resolved from settings, link_preview_options) that only that
    function applies."""
    from aiogram.types import LinkPreviewOptions

    bot = AsyncMock()
    candidate = _synthesized_candidate()

    await send_event_recap_review(bot, candidate, dry_run=False)

    _, kwargs = bot.send_message.call_args
    assert kwargs["parse_mode"] == ParseMode.HTML
    assert kwargs["link_preview_options"] == LinkPreviewOptions(is_disabled=True)


@pytest.mark.asyncio
async def test_publishable_is_never_touched_by_the_notifier() -> None:
    """EventRecapCandidate.publishable stays exactly what services/event_recap.py already
    returned - this notifier never reads it, never sets it, never returns a mutated candidate at
    all (its own return type is RoutingOutcome, not EventRecapCandidate)."""
    bot = AsyncMock()
    candidate = _synthesized_candidate()
    assert candidate.publishable is False

    outcome = await send_event_recap_review(bot, candidate, dry_run=False)

    assert candidate.publishable is False  # unchanged - candidate is frozen, was never mutated
    assert not hasattr(outcome, "publishable")


@pytest.mark.asyncio
async def test_accepts_an_explicit_alternate_destination(monkeypatch: pytest.MonkeyPatch) -> None:
    """The caller may pass a different EditorialDestination explicitly - this module makes no
    destination decision of its own beyond the TELEGRAPH default (module docstring)."""
    monkeypatch.setattr(settings, "news_topic_id", 11)
    bot = AsyncMock()
    candidate = _synthesized_candidate()

    await send_event_recap_review(bot, candidate, dry_run=False, destination=EditorialDestination.NEWS)

    _, kwargs = bot.send_message.call_args
    assert kwargs["message_thread_id"] == 11
